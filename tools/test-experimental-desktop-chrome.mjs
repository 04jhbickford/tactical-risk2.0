...
assert.equal(GAME_VERSION, 'V2.81.57-unified.16', 'stamp is unified.2');
...
assert.equal(resolveUxMode('?ux=three'), UX_CLASSIC, '?ux=three maps to Classic');
...
assert.match(html, /tr-game-version" content="V2\.81\.57-unified\.16"/);
assert.match(html, /style\.css\?v=V2\.81\.57-unified\.16/);
assert.match(html, /src\/main\.js\?v=V2\.81\.57-unified\.16/);
...
assert.doesNotMatch(classicLobby, /data-action="ux-classic"/);
assert.doesNotMatch(classicLobby, /data-action="ux-three"/);
