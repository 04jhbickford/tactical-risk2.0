// Experimental desktop chrome (.11). CSS-only progressive enhancement.
// Phone ≤480 / @430 density from .6/.10 stays the base tree.
// Classic Canvas path is not rewritten. Rules / dual-fork stay .10.
// Run: node tools/test-experimental-desktop-chrome.mjs

import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { GAME_VERSION } from '../src/version.js';
import { UX_LABEL_EXPERIMENTAL, resolveUxMode, UX_CLASSIC, UX_THREE } from '../src/map/presentationMode.js';
import { injectThreeChrome, syncThreeShellWidth } from '../src/map/threeMapChrome.js';

assert.equal(GAME_VERSION, 'V2.81.57-dual-path.11', 'stamp is dual-path.11');
assert.equal(UX_LABEL_EXPERIMENTAL, 'Experimental UX');
assert.equal(resolveUxMode(''), UX_CLASSIC, 'queryless stays Classic');
assert.equal(resolveUxMode('?ux=three'), UX_THREE);

const chromeSrc = readFileSync(new URL('../src/map/threeMapChrome.js', import.meta.url), 'utf8');
const classicLobby = readFileSync(new URL('../src/ui/lobby.js', import.meta.url), 'utf8');
const classicCanvas = readFileSync(new URL('../src/map/mapRenderer.js', import.meta.url), 'utf8');
const classicStyle = readFileSync(new URL('../style.css', import.meta.url), 'utf8');
const playSrc = readFileSync(new URL('../src/map/threeSoloPlay.js', import.meta.url), 'utf8');
const html = readFileSync(new URL('../index.html', import.meta.url), 'utf8');

assert.match(html, /tr-game-version" content="V2\.81\.57-dual-path\.11"/);
assert.match(html, /style\.css\?v=V2\.81\.57-dual-path\.11/);
assert.match(html, /src\/main\.js\?v=V2\.81\.57-dual-path\.11/);

assert.match(chromeSrc, /@media \(min-width: 768px\)/, 'tablet lobby multi-col');
assert.match(chromeSrc, /@media \(min-width: 1024px\)/, 'desktop rail breakpoint');
assert.match(chromeSrc, /--three-rail:min\(340px, 28vw\)/);
assert.match(chromeSrc, /#three-bottom \{[\s\S]*left:auto; bottom:0;/);
assert.match(chromeSrc, /#three-lobby \.three-lobby-actions \{[\s\S]*grid-template-columns:repeat\(3/);
assert.match(chromeSrc, /#three-lobby \.three-lobby-setup \{[\s\S]*grid-template-columns:minmax\(0, 1fr\) 280px/);
assert.match(chromeSrc, /#three-battle \.three-lanes \{[\s\S]*grid-template-columns:1fr 1fr/);
assert.match(chromeSrc, /@media \(hover: hover\) and \(pointer: fine\)/);
assert.match(chromeSrc, /syncThreeShellWidth/);

assert.match(chromeSrc, /@media \(max-width:430px\)/, 'phone density tree stays');
assert.match(chromeSrc, /#three-lobby \.three-lobby-seat \{ min-height:44px/);
assert.match(chromeSrc, /#three-confirm, #three-confirm\.is-idle, #three-confirm:disabled \{[\s\S]*min-height:56px/);
assert.match(chromeSrc, /#three-sheet-stack \{[\s\S]*pointer-events:none/);
assert.match(chromeSrc, /compactLocalSeatHtml/);
assert.match(chromeSrc, /three-lobby-footer/);

assert.doesNotMatch(classicLobby, /--three-rail/);
assert.doesNotMatch(classicCanvas, /--three-rail/);
assert.doesNotMatch(classicStyle, /--three-rail/);
assert.equal(classicCanvas.includes('Experimental UX'), false);
assert.equal(classicLobby.includes('compactLocalSeatHtml'), false);

assert.match(playSrc, /applyTerritoryCapture|applyHits/, '.10 combat fork still in ship');
assert.match(playSrc, /legalPlaceDests/, '.10 mobilize dests still in ship');
assert.doesNotMatch(playSrc, /--three-rail/, 'rules fork not used for chrome');

const { JSDOM } = await import('jsdom').catch(() => ({ JSDOM: null }));

function paintAt(width) {
  if (!JSDOM) return null;
  const dom = new JSDOM('<!doctype html><html><head></head><body></body></html>', {
    url: 'http://127.0.0.1/?ux=three',
  });
  Object.defineProperty(dom.window, 'innerWidth', { configurable: true, value: width });
  globalThis.window = dom.window;
  globalThis.document = dom.window.document;
  injectThreeChrome({ seat: 'Russians', ipc: 24, phase: 'PLACE' });
  return { document: dom.window.document, bp: syncThreeShellWidth() };
}

const phone = paintAt(390);
const tablet = paintAt(768);
const desktop = paintAt(1280);

if (phone && tablet && desktop) {
  assert.equal(phone.bp, 'phone');
  assert.equal(tablet.bp, 'tablet');
  assert.equal(desktop.bp, 'desktop');
  assert.equal(phone.document.documentElement.dataset.threeShell, 'phone');
  assert.equal(desktop.document.documentElement.dataset.threeShell, 'desktop');
  assert.ok(phone.document.getElementById('three-bottom'), 'phone still mounts bottom dock');
  assert.ok(desktop.document.getElementById('three-bottom'), 'desktop keeps same dock node');
  assert.ok(phone.document.getElementById('three-lobby'));
} else {
  console.warn('jsdom paint skipped');
}

console.log('test-experimental-desktop-chrome: PASS');
